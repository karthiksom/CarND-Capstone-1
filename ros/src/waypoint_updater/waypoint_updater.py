...
class WaypointUpdater(object):
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if self.pose and self.base_lane:
                ...
                #rospy.logwarn('self.pose working')
    ...
    def generate_lane(self):
        ...
        rospy.logwarn('stopline %s farthest %s', self.stopline_wp_idx, farthest_idx)
    ...
    def velocity_cb(self, msg):
        #rospy.logwarn('getting  velocity data')
        self.current_vel = msg.twist.linear.x

    def pose_cb(self, msg):
        #rospy.logwarn('getting  wpu  pose data')
        self.pose = msg

    def waypoints_cb(self, waypoints):
        #rospy.logwarn('getting  waypoint data')
        self.base_lane = waypoints
        if not self.waypoints_2d:
            self.waypoints_2d = [[waypoint.pose.pose.position.x, waypoint.pose.pose.position.y] for waypoint in waypoints.waypoints]
            #rospy.logwarn('getting  waypoint data - waypoint_tree')
            self.waypoint_tree = KDTree(self.waypoints_2d)

    def traffic_cb(self, msg):
        self.stopline_wp_idx = msg.data
        #rospy.logwarn('getting  traffic data - Stop line %s' , msg.data)
